...
class SektorPertanianController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $jenis = JenisKomoditas::where('sektor_id','1')->get();

        $komoditas = DB::table('komoditas')
        ->join('jenis_komoditas as jk', 'komoditas.jenis_id', '=', 'jk.id')
        ->where('komoditas.periode_id', $periode_id)
        ->where('komoditas.sektor_id','1')
        ->select('komoditas.*', 'jk.jenis_komoditas','jk.id AS jk_id')
        ->get();

        return view('tabel.pertanian',['title'=>'Sektor Pertanian','komoditas_sektor_pertanian'=>$komoditas,'periode'=>$periode,'periode_id'=>$periode_id,'periode_sekarang'=>$periode_sekarang,'jenis'=>$jenis]);
    }
